 ...
 * @since   JDK1.6
 */
public class TmxEntityScanner2 {

	private TmxEntity entity = null;
	private boolean finish = false;
	private boolean hasReader = true;
	
	public TmxEntityScanner2(TmxEntity entity) {
		this.entity = entity;
	}
	...
	public TmxEntityScanner2(XMLStringBuffer buf) {
	...
	char peekChar() throws IOException, TmxEndEntityException {
	...
	char scanChar() throws IOException, TmxEndEntityException {
		char c = peekChar();
		entity.position++;
	...
	String scanName() throws IOException, TmxEndEntityException {
		if (entity.position == entity.count) {
			load(0);
		}
	...
	boolean scanQName(QName qname) throws IOException, TmxEndEntityException {
	...
	int scanLiteral(int quote, XMLString xs) throws IOException, TmxEndEntityException {
	...
	int scanContent(XMLString xs) throws IOException, TmxEndEntityException {
	...
	boolean skipChar(char ch) throws IOException, TmxEndEntityException {
		if (entity.position == entity.count) {
			load(0);
		}
		int cc = entity.ch[entity.position];
	...
	boolean skipString(String str) throws IOException, TmxEndEntityException {
...
//	}
	...
	boolean arrangeCapacity(int length, boolean changeEntity) throws IOException, TmxEndEntityException {
	...
	boolean load(int offset) throws IOException, TmxEndEntityException {
		...
		if (count != -1) {
			...
			entity.position = offset;
			finish = true;
			throw new TmxEndEntityException();
		}
		return false;
	}
	...
	public void close() {
		if (entity != null && entity.reader != null) {
			try {
				entity.reader.close();
			} catch (IOException e) {}
		}
