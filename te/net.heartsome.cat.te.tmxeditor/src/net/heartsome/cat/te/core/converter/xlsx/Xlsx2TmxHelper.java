...
public class Xlsx2TmxHelper {
	...
	private boolean validateAppend() {
		if (tmxWriter instanceof AppendTmxWriter) {
			AppendTmxWriter appendTmxWriter = (AppendTmxWriter) tmxWriter;
			if (appendTmxWriter.canAppend(getSrcLang())) {
				appendTmxWriter.startAppend();
				return true;
			} else {
				return false;
			}
