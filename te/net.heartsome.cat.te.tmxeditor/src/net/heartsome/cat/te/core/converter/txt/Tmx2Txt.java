...
public class Tmx2Txt extends AbstractTmx2File {
	...
	private int langIndex = 0;// *all* 无明确源语言
	...
	public void doCovnerter(String tmxFile, File targetFile, IProgressMonitor monitor) throws Exception {
		...
		//1. 解析文件
		monitor.beginTask(Messages.getString("converter.common.monitor.info.start"), works);
		...
		try {
			langIndex = "*all*".equalsIgnoreCase(langs.get(0)) ? 1 : 0;
			
			for (int i = langIndex; i < langs.size(); i++) {
				bos.write(langs.get(i).getBytes());
				if (i != langs.size() - 1) {
					bos.write(SEPARATOR);
				}
			}
			...
			loop: while (true) {
				...
				case TmxReaderEvent.NORMAL_READ:
					map.clear();// 清除
					if (langIndex < 1 && event.getTu().getSource() != null) { // not *all*
					...
					// 生成每列
					for (int i = langIndex; i < langs.size(); i++) {
						String lang = langs.get(i).toLowerCase();
						if (map.containsKey(lang)) {
							bos.write(encodeTab(TextUtil.resetSpecialString(map.get(lang).getPureText()), null));
						}
						if (i != langs.size() - 1) {
							bos.write(SEPARATOR);
						}
